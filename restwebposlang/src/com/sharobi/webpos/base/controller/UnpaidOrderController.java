...
public class UnpaidOrderController {
	...
	@Autowired UnpaidOrderService unpaidorderService;
	...
	public ModelAndView loadUnapidOrder(Model model,HttpSession session)
	{
		...
		List<Order> allunpaidOrderList=unpaidorderService.getAllUnpaidOrderList(customer.getStoreId(), getTodayDate(),selLang);
	...
	@RequestMapping(value="/getunpaidorderwithLang/{orderId}",method=RequestMethod.GET)
	public void getunpaidOrderByIdWithLang(@PathVariable("orderId") int orderId,HttpSession session,HttpServletResponse response)throws IOException
	{
		logger.debug("in getunpaidorder...{}",orderId);
		Customer customer;
		if((customer=(Customer) session.getAttribute(Constants.LOGGED_IN_USER))!=null)
		{
			...
			out.print(unpaidorderService.getUnpaidOrderByIdWithLang(orderId,setLang));
			out.flush();
		}
	}
