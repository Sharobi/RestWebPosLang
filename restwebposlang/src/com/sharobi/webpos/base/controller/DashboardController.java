...
public class DashboardController {
		...
		@RequestMapping(value = "/viewroombookingdashboard",method = RequestMethod.GET)
    public ModelAndView viewroombookingdashboard(Model model,HttpSession session) {
      logger.debug("In viewroombookingdashboard METHOD AND IN DashboardController.java CLASS ......");
      ModelAndView mav = new ModelAndView();
      Customer customer = null;
      if ((customer = (Customer) session.getAttribute(Constants.LOGGED_IN_USER)) == null) {
        mav.setViewName(ForwardConstants.REDIRECT_LOGIN_PAGE);
      ...
      session.setAttribute("sescategoryList", menucategoryList);
        mav.addObject(Constants.ADMIN,"Y");
      mav.setViewName(ForwardConstants.VIEW_ROOM_BOOKING_DASHBOARD);
      return mav;
    }
