...
public class OrderService {
	...
	{
		try{
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_ORDER_SAVE);
	...
	public List<Order> getUnpaidOrderList(int storeId,String toDay,String lang)
	{
		List<Order> allUnpaidOrderList=new ArrayList<Order>();
		try{
			logger.debug("Input [store id] [to day date]: {},{}",storeId,toDay);
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_ORDER_GETUNPAIDORDERLIST).replace("{1}", String.valueOf(storeId)).replace("{2}", String.valueOf(toDay)).replace("{3}", String.valueOf(lang));
			logger.debug("url....{}",url);
	...
	public String getPaidOrderList(int storeId,String toDay)
	{
		...
		try{
			logger.debug("Input [store id] [to day date]: {},{}",storeId,toDay);
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_ORDER_GETALLPAIDORDER).replace("{1}", String.valueOf(storeId)).replace("{2}", String.valueOf(toDay));
			logger.debug("url....{}",url);
			ClientResponse response=WebServiceClient.callGet(url);
			String responseString=response.getEntity(String.class);
			logger.debug("Response string in get paid orderlist list: {}", responseString);
			//allUnpaidOrderList=gson.fromJson(responseString, new TypeToken<List<Order>>() {}.getType());
			//logger.debug("all unpaid order List fetched: {}", allUnpaidOrderList);
			return responseString;
		}catch(Exception ex)
	...
	{
		try{
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_BILL_PAYMENTINFO).replace("{1}", String.valueOf(orderNo));
			logger.debug("url....{}",url);
			ClientResponse response=WebServiceClient.callGet(url);
			String responseString=response.getEntity(String.class);
			logger.debug("Response string in get payment info: {}", responseString);
			return responseString;
		}catch(Exception ex)
		{
			logger.debug("Exception",ex);
	...
	public List<Payment> getPaymentListById(int orderNo)
	{
		...
		try{
			...
			String responseString=response.getEntity(String.class);
			logger.debug("Response string in get payment info: {}", responseString);
			payemntList=gson.fromJson(responseString, new TypeToken<List<Payment>>() {}.getType());
			return payemntList;
		}catch(Exception ex)
		{
			logger.debug("Exception",ex);
	...
	public String getPaymentTypeByStore(int storeid) {
		try {
			String url = CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT) + CommonResource.getProperty(CommonResource.WEBSERVICE_ORDER_GETPAYMENTTYPEBYSTORE).replace("{1}", String.valueOf(storeid));
			logger.debug("url....{}", url);
			ClientResponse response = WebServiceClient.callGet(url);
			String responseString = response.getEntity(String.class);
			logger.debug("Response string in get payment info: {}", responseString);
	...
	{
		try{
		ClientResponse response=WebServiceClient.callPost(CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_BILL_PAYMENT), gson.toJson(payment));
		String responseString=response.getEntity(String.class);
	...
	{
		try{
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_ORDER_BILL_PRINT).replace("{1}", String.valueOf(orderId)).replace("{2}", String.valueOf(storeId));
			logger.debug("url....{}",url);
			ClientResponse response=WebServiceClient.callGet(url);
			String responseString=response.getEntity(String.class);
	...
	public Order getOrderByIdForSpiltBill(int orderId)
	{
		...
		try{
			logger.debug("Input [order id] : {}",orderId);
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_UNPAID_ORDER_GETORDERBYID_FOR_SPLITBILL).replace("{1}", String.valueOf(orderId));
			logger.debug("url....{}",url);
			ClientResponse response=WebServiceClient.callGet(url);
			String responseString=response.getEntity(String.class);
			order=gson.fromJson(responseString, new TypeToken<Order>() {}.getType());
	...
	public Bill getBillById(int orderId)
	{
		...
		try{
			logger.debug("Input [order id] : {}",orderId);
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_BILL_GETBILLBYID).replace("{1}", String.valueOf(orderId));
			logger.debug("url....{}",url);
			ClientResponse response=WebServiceClient.callGet(url);
			String responseString=response.getEntity(String.class);
			bill=gson.fromJson(responseString, new TypeToken<Bill>() {}.getType());
	...
	{
		try{
			ClientResponse response=WebServiceClient.callPost(CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_BILL_ADDDISCOUNT), gson.toJson(discount));
			String responseString=response.getEntity(String.class);
			logger.debug("Response string: {}", responseString);
			return responseString;
			}catch(Exception ex)
	...
	public String getsplnotebyItemId(int itemId,int storeId)
	{
		...
		try{
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_ORDER_GETSPLNOTEBYITEMID).replace("{1}", String.valueOf(itemId)).replace("{2}", String.valueOf(storeId));
			logger.debug("url....{}",url);
			ClientResponse response=WebServiceClient.callGet(url);
			String responseString=response.getEntity(String.class);
			logger.debug("Response string in get specialnote by item id info: {}", responseString);
			//menuitemnoteList=gson.fromJson(responseString, new TypeToken<List<MenuItemNote>>() {}.getType());
			return responseString;
		}catch(Exception ex)
		{
			logger.debug("Exception",ex);
	...
	public String getsplnoteforStore(int storeId)
	{
		//List<MenuItemNote> menuitemnoteList=new ArrayList<MenuItemNote>();
		try{
	...
	public String getCustDetails(String contact,
			int storeId			){
		//List<MenuItemNote> menuitemnoteList=new ArrayList<MenuItemNote>();
		try{
			String url = CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT) + (CommonResource.getProperty(CommonResource.WEBSERVICE_ORDER_GETCUSTOMER_BYCONTACT).replace("{1}", String.valueOf(storeId)).replace("{2}", String.valueOf(contact)));
			logger.debug("url....{}",url);
			ClientResponse response=WebServiceClient.callGet(url);
			String responseString=response.getEntity(String.class);
			logger.debug("Response string in get specialnote by item id info: {}", responseString);
	...
	public List<String> getOrderCustContacts(int storeId){
		...
		try{
			...
			String url = CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT) + (CommonResource.getProperty(CommonResource.WEBSERVICE_ORDER_GETCUSTOMERCONTACT).replace("{1}", String.valueOf(storeId)));
			logger.debug("url....{}",url);
			ClientResponse response=WebServiceClient.callGet(url);
			String responseString=response.getEntity(String.class);
			logger.debug("Response string in get customer contact info: {}", responseString);
			custContact=gson.fromJson(responseString, new TypeToken<List<String>>() {}.getType());
			return custContact;
		}catch(Exception ex)
		{
			logger.debug("Exception",ex);
...
public String getLocList(int storeId,String loc){
	...
	/*public Order getCustDetails(	String contact,
			int storeId			) {

		String url = CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT) + (CommonResource.getProperty(CommonResource.WEBSERVICE_ORDER_GETCUSTOMER_BYCONTACT).replace("{1}", String.valueOf(contact)).replace("{2}", String.valueOf(storeId)));
		ClientResponse response = WebServiceClient.callGet(url);
		logger.debug("Response : {}", response);
		String responseString = response.getEntity(String.class);
		logger.debug("Response string: {}", responseString);
		Order custDetails = (Order) gson.fromJson(responseString, new TypeToken<Order>() {}.getType());
		logger.debug("InventoryItem Obj. List :: {}", custDetails);
	...
	public List<StoreCustomers> getallcreditCustomers(int storeId)
	{
		List<StoreCustomers> creditcustomerList=new ArrayList<StoreCustomers>();
				try{
					...
					ClientResponse response=WebServiceClient.callGet(url);
					String responseString=response.getEntity(String.class);
					logger.debug("Response string in get all credit customers: {}", responseString);
					creditcustomerList=gson.fromJson(responseString, new TypeToken<List<StoreCustomers>>() {}.getType());
...
//		List<MenuItemNote> menuitemnoteList=new ArrayList<MenuItemNote>();
		try{
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_ORDER_UPDATE_CREDITSALE).replace("{1}", String.valueOf(orderid)).replace("{2}", String.valueOf(storeId)).replace("{3}", String.valueOf(custid));
			logger.debug("url....{}",url);
			ClientResponse response=WebServiceClient.callGet(url);
			String responseString=response.getEntity(String.class);
			logger.debug("Response string in updateCreditSaleStatus : {}", responseString);
//			menuitemnoteList=gson.fromJson(responseString, new TypeToken<List<MenuItemNote>>() {}.getType());
			return responseString;
		}catch(Exception ex)
		{
	...
	{
		try{
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_ORDER_UPDATE_PAX).replace("{1}", String.valueOf(orderid)).replace("{2}", String.valueOf(pax));
			logger.debug("url....{}",url);
			ClientResponse response=WebServiceClient.callGet(url);
			String responseString=response.getEntity(String.class);
	...
	public String updateTable(int orderid,String tabno)
	{
		try{
			...
			Order ord = new Order();
			ord.setId(orderid);
			ord.setTable_no(tabno);
			/*String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_ORDER_UPDATE_TABLE).replace("{1}", String.valueOf(orderid)).replace("{2}", String.valueOf(tabno));
			logger.debug("url....{}",url);
			ClientResponse response=WebServiceClient.callGet(encodeURL);*/
			
			
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_ORDER_UPDATE_TABLE);
			logger.debug("url....{}",url);
			ClientResponse response=WebServiceClient.callPost(url, gson.toJson(ord));
			String responseString=response.getEntity(String.class);
	...
	{
		try{
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_BILL_SPLIT_M);
			logger.debug("url....{}",url);
			ClientResponse response=WebServiceClient.callPost(url, gson.toJson(splitBill));
			String responseString=response.getEntity(String.class);
	...
	{
		try{
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_BILL_SPLIT_PRINT).replace("{1}", String.valueOf(orderId)).replace("{2}", String.valueOf(storeId));
			logger.debug("url....{}",url);
			ClientResponse response=WebServiceClient.callGet(url);
			String responseString=response.getEntity(String.class);
			logger.debug("Response string in print split bill: {}", responseString);
			return responseString;
		}catch(Exception ex)
		{
			logger.debug("Exception",ex);
	...
	{
		try{
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_ORDER_KOT_CHECKLIST_PRINT).replace("{1}", String.valueOf(orderId)).replace("{2}", String.valueOf(storeId));
			logger.debug("url....{}",url);
			ClientResponse response=WebServiceClient.callGet(url);
			String responseString=response.getEntity(String.class);
			logger.debug("Response string in print kot checklist: {}", responseString);
			return responseString;
		}catch(Exception ex)
		{
			logger.debug("Exception",ex);
	...
	public String getBillWiseItemList(int orderId)
	{
		try{
			logger.debug("Input [order id] : {}",orderId);
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_SPLIT_BILL_MANUAL_BYORDERID).replace("{1}", String.valueOf(orderId));
			logger.debug("url....{}",url);
			ClientResponse response=WebServiceClient.callGet(url);
			String responseString=response.getEntity(String.class);
			return responseString;
	...
	public String getOrderWithPayment(int orderId, int storeId){
		...
		try {
			 ...
			 logger.debug("Input [order id][stode id] : {}{}{}",orderId,storeId);

			 String url = CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_PRINT_BILL_CASH).replace("{1}", String.valueOf(orderId)).replace("{2}", String.valueOf(storeId));
			 logger.debug("url....{}",url);
			 ClientResponse response=WebServiceClient.callGet(url);
			 String responseString=response.getEntity(String.class);
			 logger.debug("responseString : {}{}{}",responseString);
			  // Order order = (Order) gson.fromJson(responseString, new TypeToken<Order>() {}.getType());
			 return responseString;
	...
	public String updateServiceCharge(String orderid, int storeId, double scgargerate) {
		try{
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_ORDER_UPDATESCHARGERATE).replace("{1}", String.valueOf(orderid)).replace("{2}", String.valueOf(storeId)).replace("{3}", String.valueOf(scgargerate));
			logger.debug("url....{}",url);
			ClientResponse response=WebServiceClient.callGet(url);
			String responseString=response.getEntity(String.class);
			logger.debug("Response string in updateschargerate : {}", responseString);
			return responseString;
		}catch(Exception ex)
		{
			logger.debug("Exception",ex);
	...
	public String getPaidOrderDetailsDataById(String orderid, int storeId) {
		...
		String responseString = response.getEntity(String.class);
		logger.debug("Response string: {}", responseString);
		//List<Order> PaidOrder = (List<Order>) gson.fromJson(responseString, new TypeToken<List<Order>>() {}.getType());
		//logger.debug("PaidOrder Obj. List :: {}", PaidOrder);
