...
public class TableService {
	...
	public List<MenuCategory> getDirectCatList(int storeId)
	{
		List<MenuCategory> menuCategoryList=new ArrayList<MenuCategory>();
		try{
			logger.debug("Input [store id]: {}",storeId);
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_TABLE_MENUCAT_DIRECTCAT).replace("{1}", String.valueOf(storeId));
			logger.debug("get MenuCategory list URL: {}", url);
			ClientResponse response=WebServiceClient.callGet(url);
			String responseString=response.getEntity(String.class);
			logger.debug("Response string in get MenuCategory list: {}", responseString);
			menuCategoryList=gson.fromJson(responseString, new TypeToken<List<MenuCategory>>() {}.getType());
			logger.debug("MenuCategory List fetched: {}", menuCategoryList);
