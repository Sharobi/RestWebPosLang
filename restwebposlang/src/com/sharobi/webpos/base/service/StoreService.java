...
//			KeyBean keybean=gson.fromJson(responseString, KeyBean.class);
	...
	public String saveRestOpentime(int id,int userid,int restopentimeid)
	{
		try{
			logger.debug("Input [store id]: {}", id);
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_STORELOCATOR_INSERTOPENTIME).replace("{1}", String.valueOf(id)).replace("{2}", String.valueOf(userid)).replace("{3}", String.valueOf(restopentimeid));
			logger.debug("get store by id URL: {}", url);
			ClientResponse response=WebServiceClient.callGet(url);
			String responseString=response.getEntity(String.class);
			logger.debug("Response saveRestOpentime: {}", responseString);
			return responseString;
		}catch (Exception e) {
			logger.error("Exception: ", e);
			return null;
	...
	public String checkOpentimestatus(int storeid)
	{
		try{
			logger.debug("Input [store id]: {}", storeid);
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_STORELOCATOR_CHECKOPENTIMESTATUS).replace("{1}", String.valueOf(storeid));
			logger.debug("get store by id URL: {}", url);
			ClientResponse response=WebServiceClient.callGet(url);
			String responseString=response.getEntity(String.class);
			logger.debug("Response checkOpentimestatus: {}", responseString);
			return responseString;
		}catch (Exception e) {
			logger.error("Exception: ", e);
			return null;
	...
	public String saveRestClosetime(int id,int userid,int restopentimeid)
	{
		try{
			logger.debug("Input [store id]: {}", id);
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_STORELOCATOR_INSERTCLOSETIME).replace("{1}", String.valueOf(id)).replace("{2}", String.valueOf(userid)).replace("{3}", String.valueOf(restopentimeid));
			logger.debug("get store by id URL: {}", url);
			ClientResponse response=WebServiceClient.callGet(url);
			String responseString=response.getEntity(String.class);
			logger.debug("Response saveRestOpentime: {}", responseString);
	...
	public StoreSMSConfiguration getSmsData(int storeId) {
		try{
			logger.debug("Input [store id]: {}", storeId);
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_STORE_SMS_DATA).replace("{1}", String.valueOf(storeId));
			logger.debug("get store by id URL: {}", url);
			ClientResponse response=WebServiceClient.callGet(url);
			String responseString=response.getEntity(String.class);
			logger.debug("Response string in get store sms data by id: {}", responseString);
			StoreSMSConfiguration smsData=gson.fromJson(responseString, StoreSMSConfiguration.class);
			return smsData;
		}catch (Exception e) {
			logger.error("Exception: ", e);
			return null;
