...
public class UnpaidOrderMgntController {
	...
	@Autowired  AdminUnpaidOrderService unpaidorderService;
	...
	public ModelAndView loadUnapidOrder(Model model, HttpSession session) {
		...
		List<Order> allunpaidOrderList = unpaidorderService.getAllUnpaidOrderList(customer.getStoreId(),
				getTodayDate());
	...
	@RequestMapping(value = "/loadunpaidorderperiodwise", method = RequestMethod.GET)
	public ModelAndView loadunpaidorderperiodwise(Model model, HttpSession session) {
		ModelAndView mav = new ModelAndView();
		Customer customer = null;
		if ((customer = (Customer) session.getAttribute(Constants.LOGGED_IN_USER)) == null) {
			mav.setViewName(ForwardConstants.REDIRECT_LOGIN_PAGE);
		...
		System.out.println("___is_acc_in order___" + store.getIs_account());
		mav.addObject("is_acc", store.getIs_account());
		mav.setViewName(ForwardConstants.VIEW_UNPAID_ORDER_MGNT_PAGE_PERIODWISE);
		return mav;
	}

	@RequestMapping(value = "/unpaidorderSpecificDate", method = RequestMethod.GET)
	public ModelAndView addMenuItems(@RequestParam String storeId, @RequestParam String date,
		...
		session.setAttribute(Constants.ALL_UNPAIDORDER_LIST_ADMIN, allunpaidOrderList);
		mav.addObject("allunpaidOrderList", allunpaidOrderList);
	  System.out.println("order data" +gson.toJson(allunpaidOrderList));
	...
	@RequestMapping(value = "/unpaidorderPeriodWise", method = RequestMethod.GET)
	public ModelAndView unpaidorderPeriodWise(@RequestParam String storeId, @RequestParam String fromdate,
		...
		List<Order> allunpaidOrderList = unpaidorderService.getAllUnpaidOrderListPeriodWise(customer.getStoreId(),
				fromdate, todate);
		session.setAttribute(Constants.ALL_UNPAIDORDER_LIST_ADMIN, allunpaidOrderList);
		mav.addObject("allunpaidOrderList", allunpaidOrderList);		
