...
public class MenuMgntController {
	...
	@RequestMapping(value="/viewmenumgnt",method=RequestMethod.GET)
	...
	/*@RequestMapping(value="/addmenucategory/{catName}/{bgColor}",method=RequestMethod.GET)
	public void addMenuCategory(@PathVariable("catName") String catName,@PathVariable("bgColor") String bgColor,HttpSession session,HttpServletResponse response,HttpServletRequest request) throws IOException
	{
	...
	public ModelAndView loadsubCategoryPage(Model model,HttpSession session)
	{
		...
		if(menuCatList.size()!=0){ // changes in 10.1.2019
	...
	@RequestMapping(value="/loadmenuitems",method=RequestMethod.GET)
	public ModelAndView loadMenuItems(HttpSession session)
	{
		...
		List<MenuCategory> menuCatList=(List<MenuCategory>) session.getAttribute("sescategoryList");
		List<MenuCategory> menusubcategoryList =null;
		List<FmcgUnit> fmcgList = null;
		if(menuCatList.size()!=0){ // change in 10.1.2019
		  MenuCategory menucategory = null;
		  menucategory=menuCatList.get(0);
	    menusubcategoryList=menumgntService.getMenuSubCategoryByStoreId(customer.getStoreId(),menucategory.getId());
	    fmcgList=menumgntService.getFmcgUnitByStoreId(customer.getStoreId());
...
/*	@RequestMapping(value="/addmenuitems",method=RequestMethod.POST)
	...
	public void getItemDetailsById(@PathVariable("id") int id,@PathVariable("storeId") int storeId,HttpSession session,HttpServletResponse response) throws IOException
	{
		...
		if((customer=(Customer) session.getAttribute(Constants.LOGGED_IN_USER))!=null)
		{
			...
			menuItems.setStoreId(customer.getStoreId());
			MenuItems res=menumgntService.getItemDetailsById(id,storeId);
			Gson gson = new Gson();
	...
	public ModelAndView loadallspecialnotes(HttpSession session)
	{
		...
		MenuCategory menuCategory=menumgntService.getSpecialNoteforMgnt(customer.getStoreId());
		MenuCategory menuCategory1=menumgntService.getAllMenuTree(customer.getStoreId());
		session.setAttribute("categoryTree", menuCategory1);
		List<MenuCategory> menuCatList=(List<MenuCategory>) session.getAttribute("sescategoryList");
		MenuCategory menucategory = null;
		MenuCategory menucatsplnote=menumgntService.getSpecialNoteByStore(customer.getStoreId());
		...
		menucategory=menuCatList.get(0);
		 menusubcategoryList=menumgntService.getMenuSubCategoryByStoreId(customer.getStoreId(),menucategory.getId());
		 fmcgList=menumgntService.getFmcgUnitByStoreId(customer.getStoreId());
		 colorList=menumgntService.getallColor();
		}
		//mav.addObject("menucatid",menuCatList.get(0).getId());
		mav.addObject("menucatList",menuCatList);
	...
	public void assignKotPrinters(@RequestBody String addPrinterString,HttpSession session,HttpServletResponse response) throws IOException
	{
		logger.debug("In assignkotprinter......{},{},{},{}",addPrinterString);
		/*printerloc=StringUtils.replace(printerloc, "|", "\\");
		System.out.println("printerloc:"+printerloc);*/
		Customer customer=null;
		if((customer=(Customer) session.getAttribute(Constants.LOGGED_IN_USER))!=null)
		{
			...
			Gson gson = new GsonBuilder().create();
			PrintKotMaster kotMstObj =gson.fromJson(addPrinterString, new TypeToken<PrintKotMaster>() {
			}.getType());
			
			kotMstObj.setStoreId(customer.getStoreId());
			String res=menumgntService.assignPrinter(kotMstObj);
			System.out.println("res:"+res);
			out.print(res);
			out.flush();
		}
	...
	public ModelAndView uploadMenuItem(@Valid @ModelAttribute("menufile") MenuFile menuFile, @RequestParam MultipartFile fileUpload,HttpSession session,HttpServletResponse response,HttpServletRequest request,BindingResult result)
	{
		...
		if (fileUpload != null && fileUpload.getSize() > 0) {
			try{
				...
				menuFile.setFile(fileUpload);
				//String res=menumgntService.uploadMenuItems(menuFile,multipartEntity);
				String res=menumgntService.uploadMenuItems(multipartEntity);
