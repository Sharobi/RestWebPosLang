...
public class TableMgntService {
	...
	public String updateTablePosition(TableMaster tableMaster)
	{
		try{
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_TABLEMGNT_TABLE_UPDATETABLEPOSITION);
			logger.debug("url....{}",url);
			ClientResponse response=WebServiceClient.callPost(url, gson.toJson(tableMaster));
			String responseString=response.getEntity(String.class);
			logger.debug("response string updateTablePosition...{}",responseString);
			return responseString;
		}
		catch(Exception ex)
		{
