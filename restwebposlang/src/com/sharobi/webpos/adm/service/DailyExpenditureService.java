...
public class DailyExpenditureService {
	...
	public String deleteexpenditure(DailyExpenditure dailyExpenditure) {
		try{
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_ACCOUNT_VIEW_DELETEEXPENDITURE);
			ClientResponse response=WebServiceClient.callPost(url, gson.toJson(dailyExpenditure));
			String responseString=response.getEntity(String.class);
			logger.debug("response String {}", responseString);
			return responseString;
