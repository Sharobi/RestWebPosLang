 ...
 */
@Service
public class MenuItemTranslationService {
	private final static Logger logger=LogManager.getLogger(MenuItemTranslationService.class);
	private final static Gson gson=new Gson();
	
	//Get menu items for translation
	public List<MenuItemLangMap> getAllMenuItemforTranslation(int storeId, String lang)
	{
		List<MenuItemLangMap> menu=new ArrayList<MenuItemLangMap>();
		try{
		logger.debug("Input [store id][]: {},{}", storeId,lang);
		ClientResponse response=WebServiceClient.callGet(CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_MENU_TRANSLATION_GETLIST).replace("{1}",String.valueOf(storeId)).replace("{2}",lang));
		String responseString=response.getEntity(String.class);
		logger.debug("Response string in get all menu list: {}", responseString);
		//MenuItemLangMap menu=gson.fromJson(responseString, MenuItemLangMap.class);
		menu=gson.fromJson(responseString, new TypeToken<List<MenuItemLangMap>>() {}.getType());
		return menu;
		}catch (Exception e) {
	...
	public String translateMenuItem(MenuItemLangMap translateMenu)
	{
		try{
			String url=CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_MENUMGNT_MENUITEM_TRANSLATEMENUITEM);
			logger.debug("url....{}",url);
			ClientResponse response=WebServiceClient.callPost(url, gson.toJson(translateMenu));
			String responseString=response.getEntity(String.class);
			logger.debug("response string...{}",responseString);
			return responseString;
		}
