...
public class InventoryService {
	...
	@Autowired 
	InventoryMgntService inventorymgntService;
	...
	public InventoryItems getInventoryItemDetails(	int storeId,
		...
		logger.debug("Response string: {}", responseString);
		InventoryItems InventoryItem = (InventoryItems) gson.fromJson(responseString, new TypeToken<InventoryItems>() {}.getType());
		logger.debug("matric unit id : {}", InventoryItem.getMetricUnitId());
		MetricUnit metricUnit = inventorymgntService.getInvItemUnitDetails(InventoryItem.getMetricUnitId());
		InventoryItem.setUnit(metricUnit.getUnit());
	...
	public String createInventoryPurchaseOrder(InventoryPurchaseOrder purchaseOrder) {
		try {
	...
	public String updateInventoryPurchaseOrder(InventoryPurchaseOrder purchaseOrder) {
		try {
  ...
  public List<ItemCurrentStock> getCurrentStockByIngredients(int storeId, int itemId) {
    List<ItemCurrentStock> ingredientsList = new ArrayList<ItemCurrentStock>();
    try {
      String url = CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT) + (CommonResource.getProperty(CommonResource.WEBSERVICE_INVENTORY_CURRENTSTOCK_ITEM).replace("{1}", String.valueOf(storeId)).replace("{2}", String.valueOf(itemId)));
      logger.debug("URL :: {}", url);
      ClientResponse response = WebServiceClient.callGet(url);
      String responseString = response.getEntity(String.class);
      logger.debug("Response :: {}", responseString);
      ingredientsList= (List<ItemCurrentStock>) gson.fromJson(responseString, new TypeToken<List<ItemCurrentStock>>() {}.getType());
