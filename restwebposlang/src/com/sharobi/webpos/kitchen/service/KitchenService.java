...
public class KitchenService {
	...
	public List<OrderItem> getKitchenItemwithLang(int storeId,String date, String lang)
	{
		List<OrderItem> kitchenitemList=new ArrayList<OrderItem>();
		try{
			logger.debug("Input [store id],date: {},{}", storeId,date,lang);
			ClientResponse response=WebServiceClient.callGet(CommonResource.getProperty(CommonResource.TARGET_WEBSERVICE_ENDPOINT)+CommonResource.getProperty(CommonResource.WEBSERVICE_KITCHEN_GETKITCHENITEM_WITH_LANG).replace("{1}", String.valueOf(storeId)).replace("{2}",date).replace("{3}", String.valueOf(lang)));
			String responseString=response.getEntity(String.class);
			logger.debug("Response string in get all kitchen item list: {}", responseString);
			kitchenitemList=gson.fromJson(responseString, new TypeToken<List<OrderItem>>() {}.getType());
			return kitchenitemList;
