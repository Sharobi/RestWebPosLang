...
function rowClicked(id) {
	...
	//$("#itemdetails").html("");
	//$("#poOrderSave").addClass('disabled');
	$("#purchaseOrderListTbl").css("pointer-events", "none");
	$("#poItemTableDiv").addClass('hide');
			...
			for ( var j = 0; j < invPOItems.inventoryPurchaseOrderItems.length; j++) {
			   ...
			   //var eighttdline = "<td id='itmtot_" + itemdetail.inventoryItems.id + "'>" + parseFloat(((itemdetail.itemQuantity * itemdetail.rate) + itemdetail.shippingCharge)).toFixed(2) + "</td>";
				...
				if (invPOItems.poStatus == 'Y' || invPOItems.approved == 'Y' ) {
					...
					var tentdline = "<td ><a class='btn btn-success' style='background: #78B626;display:none;' id='itmupdate_" + itemdetail.inventoryItems.id + "' href='javascript:editExistingPOItem(" + itemdetail.inventoryItems.id + "," + itemdetail.id + ")'>"+getInvLang.update+"</a>&nbsp;&nbsp;&nbsp; <a href='javascript:deleteRowId(" + itemdetail.inventoryItems.id + "," + invPOItems.id + "," + itemdetail.id + ")'><img border='0' alt='' src='" + BASE_URL + "/assets/images/inv/delete_normal.png' height='22' width='18' alt='delete item'></td></a>";
				...
				$("#poItemTableDiv").removeClass('hide');
				$("#waitimage").addClass('hide');
				$("#purchaseOrderListTbl").css("pointer-events", "");
				$("#itemdetails").append(createdrowline);
