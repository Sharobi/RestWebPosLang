...
function addNewStockInItem() { // modified
	...
	//var stkinclose=$("#closeStockin").val();
...
function rowClicked(id) {
	//alert("clicked");
	//$("#itemdetails").html("");
	$("#inventoryStockInListTbl").css("pointer-events", "none");
	$("#stockinItemTableDiv").addClass('hide');
	...
	ajaxCallObject.callAjax(BASE_URL + "/stockinnew/getstockIndetailsbyidanddate/" + id + "/" + selectDate + ".htm", function(response) {
		...
		for ( var i = 0; i < stockindetail.length; i++) {
			...
			for ( var j = 0; j < invstockInItems.inventoryStockInItems.length; j++) {
				...
				$("#stockinItemTableDiv").removeClass('hide');
				$("#inventoryStockInListTbl").css("pointer-events", "");
			...
			if (invstockInItems.closed == 'Y') {
				...
				$("#stockInPrint").removeClass('disabled'); // new 7.3.2018
			} else {
				$("#closeStockin").val('N');
				$("#stockinUserId").val('');
				$('#closeStockin').removeAttr('disabled');
				$("#closestockinButton").removeClass('disabled');
				//$("#selectedvendorId").removeAttr('disabled');  //modified
				$("#selectedvendorId").prop('disabled', true); //new
				$("#stockInSave").removeClass('disabled');
	...
	if (selectedstockinid != 0 && selectedstockinid != undefined) {
		...
		$('#stockinItemTable > tbody  > tr').each(function() {
			...
			if(istaxexclusive == 'Y'){
	    	    ...
	    	    totgrossamt = Number(totgrossamt) + (basePrice)
	    	 }
...
var actualgrossamt=totgrossamt.toString().match(/^-?\d+(?:\.\d{0,2})?/)[0];
		...
		inventoryStockIn.billDate=billdata;
		/*
		 * for account
		 */
		if (is_Acc=='Y') {
		...
		$.ajax({
			url : BASE_URL + "/stockinnew/stockInClosed.htm?stockInId=" + selectedstockinid,
			type : 'POST',
			contentType : 'application/json;charset=utf-8',
...
/*function grossamtcalculation(){ //11.7.2018
	var misccharge=$("#misChargeId").val();
	var discountamt=$("#discountAmtId").val();
	var taxamt=$("#totalTaxId").val();
	var netamt=$("#grandtotalId").val();
...
function stockInPrintData(){
	window.open(BASE_URL+'/pages/inventory/printstockin.jsp','_blank');
}
