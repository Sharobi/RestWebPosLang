...
function getPurchaseReturnByDate() {
	...
	location.href = BASE_URL + "/rawstockreturn/getrawPurchaseReturndetailsbydate/" + purchaseReturnDate + ".htm";
}
...
function rowClicked(id) {
	...
	ajaxCallObject.callAjax(BASE_URL + "/rawstockreturn/getrawpurchaseReturndetailsbyidanddate/" + id + "/" + selectDate + ".htm", function(response) {
...
function purchaseReturnSaveOrUpdate() {
	...
	$('#purchaseReturnItemTable > tbody  > tr').each(function() {
		...
		if (qty < 0 || rate < 0 || qty == '' || rate == '' || returnid == 0 || returnid == '' ) {
			shouldpositive = 1;
		}
	});
	if (shouldpositive == 1) {
		...
		//purchaseReturn1.createdDate=returncreatedate;
		...
		$.ajax({
			url : BASE_URL + "/rawstockreturn/createorupdaterawpurchasereturn.htm",
			type : 'POST',
			...
			success : function(response) {
				...
				if (response == 'success') {
					...
					if (purchasereturnid == 0) {
						location.href = BASE_URL+'/rawstockreturn/loadrawstockreturn.htm';
					} else {
...
function closeStatusUpdated() {
	...
	if (purchasereturnid != 0 && purchasereturnid != undefined) {
		...
		$.ajax({
			url : BASE_URL + "/rawstockreturn/rawpurchasereturnclosed.htm?purchaseReturnId=" + purchasereturnid,
			type : 'POST',
			contentType : 'application/json;charset=utf-8',
...
function stockInDelete(){ // added new
	...
	var ajaxCallObject = new CustomBrowserXMLObject();
	ajaxCallObject.callAjax(BASE_URL + '/rawstockreturn/deleterawpurchasereturnbyid/' + purchaseReturnId+'.htm' , function(response) {
		if (response == 'success') {
			location.href=BASE_URL+'/rawstockreturn/loadrawstockreturn.htm';
		}
		else{
